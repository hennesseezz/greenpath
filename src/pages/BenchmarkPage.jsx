...
function MatchBadge({ ok }) {
  return (
    <span style={{
      padding: '1px 8px', borderRadius: 5, fontSize: 11, fontWeight: 500,
...
function MetricCard({ label, value, color }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 5, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.03em' }}>{label}</div>
...
export default function BenchmarkPage() {
  ...
  const runBenchmark = async () => {
    ...
    setLog(`Done — ${sizes.length} sizes × ${runs} run(s). All costs matched: ${allOk ? 'yes ✓' : 'NO ✗'}`)
    ...
    <div>
      ...
      {summary && (
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 16 }}>
          <MetricCard label="Dijkstra exponent k"     value={summary.kd.toFixed(2)}  color="#0F6E56" />
          <MetricCard label="Bellman-Ford exponent k" value={summary.kbf.toFixed(2)} color="#854F0B" />
          <MetricCard label={`Speedup at n=${rows[rows.length-1]?.n?.toLocaleString()}`} value={`${summary.speedup}×`} color="#185FA5" />
          <MetricCard
      ...
      {rows.length > 0 && (
        <div style={{
          ...
          <div style={{ fontSize: 11, fontWeight: 500, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 10 }}>Results table</div>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr>
                  {['n','Edges','Dijkstra (ms)','Bellman-Ford (ms)','Speedup','Match'].map(h => (
                    <th key={h} style={{ textAlign: 'left', padding: '6px 10px', borderBottom: '0.5px solid var(--border)', fontWeight: 500, color: 'var(--text-muted)' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
          ...
          <div style={{ marginTop: 10, fontSize: 11, color: 'var(--text-muted)' }}>
            Seed: 42+i per size · all timing is wall-clock average over {runs} run(s) · same source (0) and target (n−1) for both algorithms
          </div>
        </div>
      )}
    </div>
